...
public class AuthService {
    ...
    private void revokeAllUserTokens(UserDetails user) {
        if (user instanceof Buyer) {
            var validUserTokens = tokenRepository.findAllValidTokenByBuyer(((Buyer) user).getBuyerId());
            ...
            var validUserTokens = tokenRepository.findAllValidTokenBySeller(((Seller) user).getSellerId());
            validUserTokens.forEach(token -> {
                token.setExpired(true);
                token.setRevoked(true);
            });
